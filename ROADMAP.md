...
## M5 — Markdown 종합 보고서 산출물

**목표**: 흩어진 산출물(콘솔 텍스트/RRG PNG/JSON·CSV/백테스트 텍스트)을 **가시성 좋은
단일 Markdown 파일**로 묶어, Claude AI 루틴(예약 클라우드 에이전트)으로 주기 수신할 수
있게 한다(스펙 8절 M5).

**작업**
- [ ] `config.yaml`/`config.py`: `report.output_dir`(기본 `reports`) 옵션 섹션(하위호환).
      `.gitignore`에 `reports/` 추가.
- [ ] `report/markdown_report.py`: `render_markdown_report` — 콘솔 리포트 전체(국면/랭킹/
      요약/거시/사이클) + RRG 차트 임베드 + 신호 안정성(백테스트) + 용어 부록을 하나의
      .md 문자열로. GFM 표는 자체 헬퍼(의존성 추가 없음), 상수/포맷터는 기존 모듈 재사용.
- [ ] `cli.py`: `--report-md [PATH]` — 차트+백테스트+md를 일괄 생성. 기본 산출 경로는
      날짜별 자기완결(`reports/flow-report-<날짜>.md` + 동명 PNG). 부분 실패는 해당 섹션만
      degrade(본 보고서를 막지 않음).

**완료 기준**
- 합성 데이터 단위테스트가 네트워크 없이 통과(섹션/표/면책 존재, degrade, 비단정).
- 한 번의 명령으로 차트·백테스트·md가 자기완결적으로 생성됨(실데이터 확인).
- 모든 출력에 후행성·면책 문구 유지.
...
**목표**: 보고서를 비전문가가 상단만 봐도 "이번 주 자금이 어디로"를 오해 없이 파악하게 하고,
티커/용어에 한글 풀이를 붙이고, 시각물(FlowScore 막대 + 실제 RRG 차트)을 더하고, 매주
노션에 날짜별 페이지로 쌓는다(스펙 3절 종합 출력 + 5절 출력 확장).

**작업**
- [x] `config.yaml`/`config.py`: `tickers.sector_desc`(섹터 한글 풀이 '한글명 — 설명') +
      `report.chart_raw_base`(노션 이미지 임베드용 GitHub raw URL) 옵션 추가(하위호환).
- [x] `report/insight.py`(신규, 순수함수): 주간 결론(`weekly_conclusion`)·읽는 법(`read_guide`)·
      FlowScore 발산 막대(`flow_bar`, −3.5~+3.5)·섹터 한글 라벨(`sector_label`). .md/노션 공유.
- [x] `report/markdown_report.py`: 상단 `## 0. 이번 주 결론` + `### 읽는 법`, 랭킹표에 한글
      라벨·분면 한글 꼬리표·흐름 막대 칼럼 추가(GFM .md, 기존 섹션 유지).
- [x] `report/notion_report.py`(신규): 같은 데이터를 Notion-flavored(`<table>`/`<callout>`/
      `<details>`)로 렌더 + RRG PNG를 raw URL로 임베드.
- [ ] 주간 노션 게시 루틴 등록(클라우드 루틴이 .md/PNG 커밋·푸시 후 Notion MCP로 날짜
      페이지 생성). 코드와 분리해 사용자 확인 후 `/schedule`로 등록.

**완료 기준**
- 합성 데이터 단위테스트가 네트워크 없이 통과(결론/막대/한글 라벨/노션 블록/degrade/비단정).
- `--report-md` 한 번으로 친화 .md가 생성됨(실데이터 확인).
- 모든 출력에 후행성·면책·비단정 원칙 유지.
