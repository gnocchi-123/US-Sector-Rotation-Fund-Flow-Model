...
- 출력: 시장 국면 → 섹터 자금흐름 랭킹표 → 핵심 요약(유입/주도/유출) → 거시 참고.
  - **사용자 친화 요건(M6)**: 사용자 대면 보고서는 상단에 "이번 주 결론"(국면 + 상대적
    유입/유출 우위 섹터)과 "읽는 법"을 두고, 티커/용어에 한글 풀이를 붙이며, FlowScore를
    발산 막대로 시각화한다. 결론은 SPY 대비 '상대 순위'임을 명시하고 단정 표현을 쓰지 않는다.
...
6. **출력**: 콘솔 표 + (선택) RRG PNG 차트 + JSON/CSV 내보내기 + (선택) Markdown 종합
   보고서(`--report-md`, 위 산출물을 가시성 좋은 단일 .md로 묶음 — M5).
...
- **M5**: Markdown 종합 보고서 산출물. 콘솔 리포트 전체 + RRG 차트 임베드 + 신호
  안정성(백테스트) + 용어 부록을 **가시성 좋은 단일 .md 파일**로 묶는다(`--report-md`).
  Claude AI 루틴(예약 클라우드 에이전트)으로 주기 수신하는 것을 전제로, 한 번의 명령으로
  전부 생성되고 이미지 링크가 항상 해석되며 실행마다 자기완결적이어야 한다. 면책·후행성·
  비단정 원칙은 기존과 동일하게 유지.
- **M6**: 친화 보고서 v2 & 노션 주간 게시. 보고서 상단에 "이번 주 결론 + 읽는 법"을 두고
  티커/용어 한글 풀이와 FlowScore 발산 막대를 더해 비전문가도 핵심을 파악하게 한다
  (`report/insight.py` 공유 헬퍼). 같은 데이터를 노션 블록(`report/notion_report.py`,
  `<table>`/`<callout>`/`<details>`)으로 렌더하고 RRG 차트를 GitHub raw URL로 임베드하며,
  매주 날짜별 노션 페이지로 쌓는다. 비단정·면책·후행성 원칙은 동일.
