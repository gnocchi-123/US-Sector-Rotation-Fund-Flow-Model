...
## M7 — 보고서 품질 점검 & 가시성 개선 (진행 중)

사용자 점검 기준: ① 비전문가 이해, ② 자금 흐름 방향 명확성, ③ 그래프·도표 가시성.
4개 약점 식별 → 우선순위 ①②부터 처리.

- [x] 1. `fix(report): RRG 차트 자동 줌` — `plot.py`. 실데이터가 RS-Ratio/RS-Mom
      100 근처(예 98~102)로 뭉쳐 라벨이 겹쳐 판독 불가하던 문제 수정. 고정 85~115
      대신 데이터 범위에 맞춰 자동 줌(100 기준선 항상 포함, 정사각형 유지, 25% 여백,
      최소 ±1). 4분면 코너 라벨은 `transAxes`로 모서리 고정, 라벨에 흰 배경 박스로
      겹침 가독성 확보. `test_plot.py`는 파일 존재만 검증 → 무영향, 179개 통과.
- [x] 2. `fix(report): .md 한글 라벨 통일` — `markdown_report.py`. §3 핵심요약·§7-1
      휩소율 표가 영어 섹터명(`Cons. Disc.`/`Technology`)을 쓰던 것을 `sector_label`
      (한글)로 통일 → 노션 보고서와 일치.

### 다음에 할 작업 (M7 잔여 — 우선순위순)

- [ ] 3. **§1 시장 국면 표 한글화** (③/① 약점). `risk["details"]`의 페어 키
      (`SmallVsLarge`/`EqualVsCap`/`CycVsDef`/`CreditRisk`/`GrowthVsValue`)와 상태값
      (`risk-ON`/`risk-OFF`/`neutral`)이 약어·영어라 비전문가가 못 읽음. 이 표가
      "왜 MIXED인지" 근거인데 가장 어려움.
      - 방안: `config.yaml`에 페어 한글 풀이 매핑(예 "중소형주 vs 대형주") 추가하고
        `insight.py`에 헬퍼(예 `pair_label`/`risk_state_ko`) 두어 .md/노션 양쪽 재사용.
        하드코딩 금지(스펙 원칙) — 매핑은 config로. 위험선호 점수 산출 키는
        `signals/risk.py` 확인 후 매핑 키 일치시킬 것.
- [ ] 4. **거시(§5)·사이클(§6) 시각화** (③ 약점). 현재 숫자 표뿐. §2처럼 발산 막대
      (🟩/🟥)를 거시 변화율·사이클 z-score에도 붙여 일관된 가시성 부여.
      `insight.flow_bar`를 일반화하거나 별도 막대 헬퍼 추가(스케일 기준 주의).
- [ ] 5. (선택) **§2 랭킹의 모순 인상 보완** (② 약점). 유입 상위인데 `Rot=D`/
      RS-Mom 100 근처라 비전문가에겐 모순처럼 보임. "이른 신호라 회전이 아직
      약한 게 정상" 같은 한 줄 주석 추가 검토.
- [ ] 6. (선택) MIXED(혼조) 국면일 때 결론에 "방향성 약함, 상대 순위만 참고" 강조.
- [ ] 7. (선택) 과거 스냅샷 `reports/flow-report-2026-06-15.png`는 줌 개선 전 차트.
      날짜별 기록이라 그대로 둠 — 개선 효과는 다음 보고서 실행부터. 필요 시 실데이터로
      해당 날짜 차트만 재생성·교체.

## 미완 (M6 잔여)

1. 주간 노션 자동 게시 루틴 등록(클라우드 루틴이 .md/PNG 커밋·푸시 후 Notion MCP로
   "자금 순환 모델" 아래 날짜 페이지 생성). 코드와 분리, 사용자 확인 후 `/schedule`.
