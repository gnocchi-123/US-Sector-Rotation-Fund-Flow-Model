...
- [ ] 4. `feat(signals): rrg / risk / trend pure functions` — **진행 중(이번 작업)**
...
- [ ] 5. `test: synthetic-data unit tests (no network)`
      — `tests/conftest.py`(합성 가격 fixture) + `tests/test_{rrg,risk,trend}.py`.
        RRG는 모멘텀 분면 대신 RS-Ratio 부호를 단언, 위험선호 ±2 임계값, 추세 분류/degrade.
...
1. 커밋 5: 합성 데이터 단위테스트 — `conftest.py` + `test_rrg.py`/`test_risk.py`/`test_trend.py`.
2. 커밋 6: `report/synthesize.py` + `report/plot.py` + `cli.py` — M1 완료.
